...
class AccountMove(models.Model):
    ...
    b_to_b = fields.Boolean(string="B2B")
    b_to_c = fields.Boolean(string="B2C")
    manual_invoice_no = fields.Char(string = "Manual Bill")
    tally_direct_estimate = fields.Boolean(string="Manual Invoice Entry")


    #
    # @api.onchange('manual_invoice_no')
    # def onchange(self, values, field_name, field_onchange):

    def brothers_sequence_new(self,inb):
        for each in inb:
            if not each.tally_direct_estimate:
                if each.b_to_b:
                    if each.branch_id:
                        if self.env['account.move'].search([('move_type', '=', each.move_type),('b_to_b', '=', each.b_to_b),('branch_id', '=', each.branch_id.id),('company_id', '=', each.company_id.id)]):
                            if len(self.env['account.move'].search([('move_type', '=', each.move_type),('b_to_b', '=', each.b_to_b),('branch_id', '=', each.branch_id.id),('company_id', '=', each.company_id.id)])) >= 2:
                                last_move_id = self.env['account.move'].search(
                                    [('branch_id', '=', each.branch_id.id),('company_id', '=', each.company_id.id), ('b_to_b', '=', each.b_to_b),
                                     ('move_type', '=', each.move_type)])[1].name.split('/')[-1]
                                #
                                # last_move_id = self.env['account.move'].search([('branch_id', '=', each.branch_id.id)])[
                                #                    1].name[-4:]
                                last_move_id = int(last_move_id)
                                invoice_name = ''
                                list = self.env['account.move'].search([('branch_id', '=', each.branch_id.id),('company_id', '=', each.company_id.id),
                                                                        ('b_to_b', '=', each.b_to_b),
                                                                        ('move_type', '=', each.move_type)])[1].name
                                for one_by_one in list.split('/')[:-1]:
                                    invoice_name += one_by_one + '/'
                                prefit = invoice_name
                                each.name = prefit  + str(last_move_id + 1)
                            else:
                                each.name = each.branch_id.code+'/'+'B/'+'1'

                        else:
                            each.name = each.branch_id.code + '/' + 'B/' + '1'
                    else:
                        if len(self.env['account.move'].search([('branch_id', '=', False),('company_id', '=', each.company_id.id),('b_to_b', '=', each.b_to_b),('move_type','=',each.move_type)])) >= 2:
                            last_move_id = self.env['account.move'].search([('branch_id', '=', False),('company_id', '=', each.company_id.id),('b_to_b', '=', each.b_to_b),('move_type','=',each.move_type)])[1].name.split('/')[-1]
                            # last_move_id = self.env['account.move'].search([('company_id', '=', each.company_id.id)])[
                            #                    1].name[-4:]
                            last_move_id = int(last_move_id)
                            invoice_name = ''
                            list = self.env['account.move'].search([('branch_id', '=', False),('company_id', '=', each.company_id.id),
                                                             ('b_to_b', '=', each.b_to_b),
                                                             ('move_type', '=', each.move_type)])[1].name
                            for one_by_one in list.split('/')[:-1]:
                                invoice_name +=one_by_one+'/'
                            ...
                            each.name = each.company_id.code + '/' + 'B/' + '1'
                if each.b_to_c:
                    if each.branch_id:
                        if self.env['account.move'].search([('tally_direct_estimate','=',False),('move_type', '=', each.move_type),('b_to_c', '=', each.b_to_c),('company_id', '=', each.company_id.id),('branch_id', '=', each.branch_id.id)]):
                            if len(self.env['account.move'].search([('tally_direct_estimate','=',False),('move_type', '=', each.move_type),('b_to_c', '=', each.b_to_c),('company_id', '=', each.company_id.id),('branch_id', '=', each.branch_id.id)])) >= 2:
                                last_move_id = self.env['account.move'].search(
                                    [('tally_direct_estimate','=',False),('branch_id', '=', each.branch_id.id),('company_id', '=', each.company_id.id), ('b_to_c', '=', each.b_to_c),
                                     ('move_type', '=', each.move_type)])[1].name.split('/')[-1]
                                #
                                # last_move_id = self.env['account.move'].search([('branch_id', '=', each.branch_id.id)])[
                                #                    1].name[-4:]
                                last_move_id = int(last_move_id)
                                invoice_name = ''
                                list = self.env['account.move'].search([('tally_direct_estimate','=',False),('branch_id', '=', each.branch_id.id),('company_id', '=', each.company_id.id),
                                                                        ('b_to_c', '=', each.b_to_c),
                                                                        ('move_type', '=', each.move_type)])[1].name
                                for one_by_one in list.split('/')[:-1]:
                                    invoice_name += one_by_one + '/'
                                ...
                                each.name = each.branch_id.code+'/'+'C/'+'1'
                        else:
                            each.name = each.branch_id.code + '/' + 'C/' + '1'

                    else:
                        if len(self.env['account.move'].search([('tally_direct_estimate','=',False),('branch_id', '=', False),('company_id', '=', each.company_id.id),('b_to_c', '=', each.b_to_c),('move_type','=',each.move_type)])) >= 2:
                            last_move_id = self.env['account.move'].search([('tally_direct_estimate','=',False),('branch_id', '=', False),('company_id', '=', each.company_id.id),('b_to_c', '=', each.b_to_c),('move_type','=',each.move_type)])[1].name.split('/')[-1]
                            # last_move_id = self.env['account.move'].search([('company_id', '=', each.company_id.id)])[
                            #                    1].name[-4:]
                            last_move_id = int(last_move_id)
                            invoice_name = ''
                            list = self.env['account.move'].search([('tally_direct_estimate','=',False),('branch_id', '=', False),('company_id', '=', each.company_id.id),
                                                             ('b_to_c', '=', each.b_to_c),
                                                             ('move_type', '=', each.move_type)])[1].name
                            for one_by_one in list.split('/')[:-1]:
                                invoice_name +=one_by_one+'/'
                            ...
                            each.name = each.company_id.code + '/' + 'C/' + '1'
            else:
                each.name = each.manual_invoice_no
...
class SaleEstimate(models.Model):
    ...
    def total_sales_create(self):
        ...
        order = self.env['sale.order'].create({
            ...
            # 'estimate_order_id': self.id,
            # 'warehouse_id': self.env['stock.warehouse'].search([('company_id', '=', self.company_id.id)]).id,
        })
        if self.est_order_id:
            self.est_order_id.write({'status':'estimate'})
        est_list = []
        for lines in self.estimate_ids:
            # lines.product_uom_qty = sum(lines.sub_customers.mapped('quantity'))
            lines.send_qty = sum(lines.sub_customers.mapped('quantity'))
            self.env['sale.order.line'].create({
                'name': lines.product_id.name,
                'product_id': lines.product_id.id,
                'product_uom_qty': lines.send_qty,
                'product_uom': lines.product_uom.id,
                'price_unit': lines.price_unit,
                'customer_lead': lines.product_id.sale_delay,
                'order_id': order.id,
                # 'tax_id': [(6, 0, lines.tax_ids.ids)],
            })
            # est_list.append(product_line)
        # self.order_line = est_list
        order.action_confirm()
        complete_address=''
        if self.partner_id.street2:

            complete_address += self.partner_id.street2 +','
        if self.partner_id.street:
            complete_address += self.partner_id.street+','
        if self.partner_id.city:
            complete_address += self.partner_id.city+','
        if self.partner_id.state_id:
            complete_address += self.partner_id.state_id.name+','
        if self.partner_id.country_id:
            complete_address += self.partner_id.country_id.name+','
        if self.partner_id.mobile:
            complete_address += self.partner_id.mobile
        self.total_sale = order
        ...
        inb.write({'b_to_b': inb.partner_id.b_to_b})
        inb.write({'b_to_c': inb.partner_id.b_to_c})
        inb.write({'tally_direct_estimate': self.tally_direct_estimate})

        if self.est_order_id:
           inb.write({'est_order_id': self.est_order_id.id})

        inb.remarks =self.remarks
        inb.brothers_sequence_new(inb)
        for line_vehicle in self.estimate_ids:
            inb.vehicle_number = line_vehicle.vahicle.license_plate
        # inb.action_invoice_open()
        inb.action_post()
        inb.complete_address =complete_address
        for pickings in order.picking_ids:
                ...
                if line.company_ids.id != 1:
                    ...
                    if line.sub_customers:
                        for sub_cus in line.sub_customers:
                            ...
                            inb.write({'b_to_b': inb.partner_id.b_to_b})
                            inb.write({'b_to_c': inb.partner_id.b_to_c})
                            inb.write({'manual_invoice_no': sub_cus.invoice_manual})
                            inb.write({'tally_direct_estimate': self.tally_direct_estimate})
                            inb.brothers_sequence_new(inb)
                            inb.remarks = self.remarks
...
class ResPartner(models.Model):
    ...
    def action_confirm(self):
        for all in self.env['res.partner'].search([('vat','!=',False)]):
            all.b_to_b = False
        for b2c_all in self.env['res.partner'].search([('vat','=',False)]):
            b2c_all.b_to_c = False
    ...
    #     for all in self.env['res.partner'].search([('vat','!=',False)]):
    #         all.b_to_b = True
    #     for c_all in self.env['res.partner'].search([('vat','=',False)]):
    #         c_all.b_to_c = True
...
class ResSubPartners(models.Model):
    ...
    invoice_manual = fields.Char(string="Manual Inv Number")
